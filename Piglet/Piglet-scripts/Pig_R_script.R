...
setwd("~/Desktop/Tim-Lab/Fecal_study/R_analysis/pig")

metadata <- read.table("pig_metadata.txt", header=TRUE, sep="\t")
str(metadata)
...
write.table(meta,file = "pig_alpha.txt",quote = F,sep = '\t', row.names = T, col.names = T)

#Mixed model
#Evenness
meta$Day_cat <- factor(meta$Day_cat, levels = c("d0", "d1", "d2", "d3", "d7", "d14", "d21"))

m1_evenness <- mixed(pielou_evenness ~ Day_cat + Animal + (1|Animal), data = meta, method = "KR",
...
ggsave(paste0("output/pig_evenness", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

#OTU
m1_otu <- mixed(observed_features ~ Day_cat + Animal + (1|Animal), data = meta, method = "KR",
                control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
ggsave(paste0("output/T_pig_OTU", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches


#Faith_pd
m1_faith <- mixed(faith_pd ~ Day_cat + Animal + (1|Animal), data = meta, method = "KR",
                  control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
ggsave(paste0("output/pig_faith", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

#Shannon
m1_shannon <- mixed(shannon_entropy ~ Day_cat + Animal + (1|Animal), data = meta, method = "KR",
                    control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
ggsave(paste0("output/pig_shannon", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

#Simpson
m1_simpson <- mixed(simpson ~ Day_cat + Animal + (1|Animal), data = meta, method = "KR",
                    control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
ggsave(paste0("output/pig_simpson", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

#Simpson_evenness
m1_simpson_e <- mixed(simpson_e ~ Day_cat + Animal + (1|Animal), data = meta, method = "KR",
                      control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
ggsave(paste0("output/pig_simpson_e", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/BC-ellipse_", my_column,"-pig.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
write.table(Uni_meta,file = "Uni_piglet.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggplot(Uni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/Un-basic_", my_column,"-calf.pdf"), height=2, width=3, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(Uni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*Uni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/Uni-ellipse_", my_column,"-pig.pdf"), height=2.5, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
write.table(Wuni_meta,file = "Wuni_piglet.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/WU-basic_", my_column,"-pig.pdf"), height=2, width=3, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*Wuni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/Wuni-ellipse_", my_column,"-pig.pdf"), height=2.5, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
for(ml in my_level){
  ...
  ggsave(paste0("output/", ml, "BarPlot_", my_column, "-pig.png"), height = 5, width = 5)
}
...
#training
training = metadata[metadata$Animal %in% c("P1","P2","P3","P4", "P5", "P6", "P7", "P8"),] 
#metadata[,1]
#metadata[1,]
...
head(imp)
write.table(imp,file = "importance_genus_rare-l6.txt",quote = F,sep = '\t', row.names = T, col.names = T)
#visulisation simple
varImpPlot(rf, main = "Top 20 - Feature OTU importance",n.var = 20, bg = par("bg"),
           color = par("fg"), gcolor = par("fg"), lcolor = "gray" )

imp = read.table("importance_genus_rare-l6.txt", header=T, row.names= 1, sep="\t") 
imp = head(imp, n=20)
imp = imp[order(1:20,decreasing = T),]
...
ggsave(paste0("output/Important_OTUs_pig_rare_RF-l6a", ".pdf"), p2, height=8, width=12, device="pdf") # save a PDF 8 inches by 12 inches
...
p4 #time vs. taxon
ggsave(paste0("output/Heat_map_rare_Day-l6", "-pig.png"), p4, height = 8, width = 16)
...
imp$order2 =  as.numeric(gsub("P1","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("P2","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("P3","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("P4","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("P5","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("P6","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("P7","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("P8","",imp$order,perl=TRUE) )
taxonomy = arrange(imp, desc(order2), genus)

otu_norm_group1 = otu_norm_group[match(taxonomy$genus,rownames(otu_norm_group)),] # 按初始排序

p5=pheatmap(otu_norm_group1,scale="row",cluster_cols = F, cluster_rows = F, fontsize_row = 17, fontsize_col = 15)
p5 #time vs taxon 
ggsave(paste0("output/Heat_map_rare_Day_taxon-l6", "-pig.png"), p5, height = 8, width = 7)

#using training set to evalue

train.p = predict(rf, type = "response")
df = data.frame(observed = training$Day, predict = train.p)
write.table(df,file = "train_predict_rare-l6.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
p6 = ggplot(df, aes(observed,predict)) +
  ...
  theme_bw()
p6
ggsave(paste0("output/Training_prediction_rare-l6", "-pig.png"), p6, height = 4, width = 4)

test_set = metadata[metadata$Animal%in% c("P9","P10"),]
...
write.table(df,file = "test_predict_rare-l6.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
p7 = ggplot(df, aes(observed,predict)) +
  ...
  theme_bw()
p7
ggsave(paste0("output/Test_prediction_rare-l6", "-pig.png"), p7, height = 4, width = 4)
...
ggsave(paste0("output/pig_picrust_OTU", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/pig_picrust_BC-ellipse_", my_column,"-pig.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
phenotypes <- read.table("pig_predictions.txt", header=TRUE, sep="\t")
str(phenotypes)

meta_phenotypes <- merge(metadata, phenotypes, by.x = "ID", by.y = "Sample_ID")
str(meta_phenotypes)

meta_phenotypes <- meta_phenotypes[-which(meta_phenotypes$ID %in% c("P_d3_P7")), ]

#Anaerobic
m1_Anaerobic <- mixed(Anaerobic ~ Day + Animal + (1|Animal), data = meta_phenotypes, method = "KR",
                control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
with(meta_phenotypes, shapiro.test(Anaerobic_sqrt))
...
ggsave(paste0("output/pig_bugbase_Anaerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/pig_bugbase_Aerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- meta_phenotypes[-which(meta_phenotypes$ID %in% c("P_d3_P7")), ]

meta_phenotypes <- mutate(meta_phenotypes, Aerobic_log = log10(Aerobic))

#Transformed Aerobic
T1_Aerobic <- mixed(Aerobic_log ~ Day + Animal + (1|Animal), data = meta_phenotypes, method = "KR",
                    control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)

summary(T1_Aerobic)

anova(T1_Aerobic) 
plot(T1_Aerobic$full_model)
qqnorm(residuals(T1_Aerobic$full_model))
emm_options(lmer.df = "asymptotic") # also possible: 'satterthwaite', 'kenward-roger'
emm_t1_Aerobic<- emmeans(T1_Aerobic, "Day")
emm_t1_Aerobic
update(pairs(emm_t1_Aerobic), by = NULL, adjust = "holm")
with(meta_phenotypes, shapiro.test(Aerobic_log))

afex_plot(T1_Aerobic, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Log transformed Relative Abundance of Aerobes", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_pig_bugbase_Anaerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
with(meta_phenotypes, shapiro.test(Facultatively_Anaerobic_sqrt))
...
ggsave(paste0("output/pig_bugbase_F_Anaerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes, Facultatively_Anaerobic_log = log10(Facultatively_Anaerobic))

#Log transformed Facultatively Anaerobic
T1_F_Anaerobic <- mixed(Facultatively_Anaerobic_log ~ Day + Animal + (1|Animal), data = meta_phenotypes, method = "KR",
                        control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)

summary(T1_F_Anaerobic)

anova(T1_F_Anaerobic) 
plot(T1_F_Anaerobic$full_model)
qqnorm(residuals(T1_F_Anaerobic$full_model))
emm_options(lmer.df = "asymptotic") # also possible: 'satterthwaite', 'kenward-roger'
emm_t1_F_Anaerobic<- emmeans(T1_F_Anaerobic, "Day")
emm_t1_F_Anaerobic
update(pairs(emm_t1_F_Anaerobic), by = NULL, adjust = "holm")
with(meta_phenotypes, shapiro.test(Facultatively_Anaerobic_log))

afex_plot(T1_F_Anaerobic, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of log Transformed Facultative Anaerobes", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_pig_bugbase_F_Anaerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
with(meta_phenotypes, shapiro.test(Forms_Biofilms))
...
ggsave(paste0("output/pig_bugbase_Biofilms", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes, Forms_Biofilms_log = log10(Forms_Biofilms))

#Log transformed Forms_Biofilms
T1_Biofilms <- mixed(Forms_Biofilms_log ~ Day + Animal + (1|Animal), data = meta_phenotypes, method = "KR",
                     control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)

summary(T1_Biofilms)

anova(T1_Biofilms) 
plot(T1_Biofilms$full_model)
qqnorm(residuals(T1_Biofilms$full_model))
emm_options(lmer.df = "asymptotic") # also possible: 'satterthwaite', 'kenward-roger'
emm_t1_Biofilms<- emmeans(T1_Biofilms, "Day")
emm_t1_Biofilms
update(pairs(emm_t1_Biofilms), by = NULL, adjust = "holm")
with(meta_phenotypes, shapiro.test(Forms_Biofilms_log))

afex_plot(T1_Biofilms, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of Log Transformed Biofilm Formers", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_pig_bugbase_Biofilms", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/pig_bugbase_Gram_Negative", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes, Gram_Negative_sqrt = sqrt(Gram_Negative))

#Sqrt Transformed Gram_Negative
T1_Gram_Negative <- mixed(Gram_Negative_sqrt ~ Day + Animal + (1|Animal), data = meta_phenotypes, method = "KR",
                          control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)

summary(T1_Gram_Negative)

anova(T1_Gram_Negative) 
plot(T1_Gram_Negative$full_model)
qqnorm(residuals(T1_Gram_Negative$full_model))
emm_options(lmer.df = "asymptotic") # also possible: 'satterthwaite', 'kenward-roger'
emm_t1_Gram_Negative<- emmeans(T1_Gram_Negative, "Day")
emm_t1_Gram_Negative
update(pairs(emm_t1_Gram_Negative), by = NULL, adjust = "holm")
with(meta_phenotypes, shapiro.test(Gram_Negative_sqrt))

afex_plot(T1_Gram_Negative, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of Square root transformed Gram_Negative", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_pig_bugbase_Gram_Negative", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

#Gram_Positive
m1_Gram_Positive <- mixed(Gram_Positive ~ Day + Animal + (1|Animal), data = meta_phenotypes, method = "KR",
                          control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
ggsave(paste0("output/pig_bugbase_Gram_Positive", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
with(meta_phenotypes, shapiro.test(Contains_Mobile_Elements_sqrt))
...
ggsave(paste0("output/pig_bugbase_Contains_Mobile_Elements", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes, Contains_Mobile_Elements_log = log10(Contains_Mobile_Elements))

#Log transformed Contains_Mobile_Elements
T1_Contains_Mobile_Elements <- mixed(Contains_Mobile_Elements_log ~ Day + Animal + (1|Animal), data = meta_phenotypes, method = "KR",
                                     control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)

summary(T1_Contains_Mobile_Elements)

anova(T1_Contains_Mobile_Elements) 
plot(T1_Contains_Mobile_Elements$full_model)
qqnorm(residuals(T1_Contains_Mobile_Elements$full_model))
emm_options(lmer.df = "asymptotic") # also possible: 'satterthwaite', 'kenward-roger'
emm_t1_Contains_Mobile_Elements<- emmeans(T1_Contains_Mobile_Elements, "Day")
emm_t1_Contains_Mobile_Elements
update(pairs(emm_t1_Contains_Mobile_Elements), by = NULL, adjust = "holm")
with(meta_phenotypes, shapiro.test(Contains_Mobile_Elements_log))

afex_plot(T1_Contains_Mobile_Elements, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of log transformed Bacteria with Mobile Elements", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_pig_bugbase_Contains_Mobile_Elements", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
