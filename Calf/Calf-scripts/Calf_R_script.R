...
metadata <- read.table("calf_metadata.txt", header=TRUE, sep="\t")
str(metadata)
...
write.table(meta,file = "calf_alpha.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggsave(paste0("output/calf_evenness", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
meta$Day <- factor(meta$Day, levels = c("d0", "d1", "d2", "d3", "d7", "d14", "d21"))
...
ggsave(paste0("output/calf_OTU", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/calf_faith", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
anova(m1_shannon) ##No significant 
...
ggsave(paste0("output/calf_shannon", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
anova(m1_simpson) ##No significant 
...
with(meta, shapiro.test(simpson_sqrt))
...
ggsave(paste0("output/calf_simpson", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
anova(m1_simpson_e) ##No significant 
...
ggsave(paste0("output/calf_simpson_e", ".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
write.table(bc_meta,file = "BC_calf.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggsave(paste0("output/BC-basic_", my_column,"-calf.pdf"), height=2, width=3, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/BC-ellipse_", my_column,"-calf.pdf"), height=2.5, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
write.table(Uni_meta,file = "Uni_calf.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggplot(Uni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*Uni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/Uni-ellipse_", my_column,"-calf.pdf"), height=2.5, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
write.table(Wuni_meta,file = "Wuni_calf.txt",quote = F,sep = '\t', row.names = T, col.names = T)
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*Wuni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/Wuni-ellipse_", my_column,"-calf.pdf"), height=2.5, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
my_column <- "Day_cat"  #this is the metadata column that we will use in the taxa barplot
...
for(ml in my_level){
  ...
  ggsave(paste0("output/", ml, "BarPlot_", my_column, "-calf.png"), height = 5, width = 5)
}
...
#training
training = metadata[metadata$Animal %in% c("C1","C2","C3","C4", "C5", "C6", "C7", "C8"),] 
#metadata[,1]
#metadata[1,]
...
x = ggplot(cross, aes(Number_OTU, result.error.cv, group = 1)) + 
  geom_line()
x =  x + geom_vline(xintercept=26, linetype="dotted", 
                    color = "blue", size=1) +
  labs(y="Cross validation error rate",
       x ="Number of ASVs") +
  theme_classic()
ggsave(paste0("output/Cross_validation_rare-l6", "-calf.png"), x, height = 4, width = 4)
...
#visulisation simple
varImpPlot(rf, main = "Top 26 - Feature OTU importance",n.var = 26, bg = par("bg"),
           color = par("fg"), gcolor = par("fg"), lcolor = "gray" )

imp = read.table("importance_genus_rare-l6.txt", header=T, row.names= 1, sep="\t") 
imp = head(imp, n=26)
imp = imp[order(1:26,decreasing = T),]
...
ggsave(paste0("output/Important_OTUs_calf_rare_RF-l6a", ".pdf"), p2, height=8, width=12, device="pdf") # save a PDF 8 inches by 12 inches
...
p4 #time vs. taxon
ggsave(paste0("output/Heat_map_rare_Day-l6", "-calf.png"), p4, height = 8, width = 16)
...
imp$order2 =  as.numeric(gsub("C1","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("C2","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("C3","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("C4","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("C5","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("C6","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("C7","",imp$order,perl=TRUE) )
imp$order2 =  as.numeric(gsub("C8","",imp$order,perl=TRUE) )
...
p5 #time vs taxon 
ggsave(paste0("output/Heat_map_rare_Day_taxon-l6b", "-calf.png"), p5, height = 8, width = 7)
...
p6 = ggplot(df, aes(observed,predict)) +
  ...
  theme_bw()
p6
ggsave(paste0("output/Training_prediction_rare-l6", "-calf.png"), p6, height = 4, width = 4)

test_set = metadata[metadata$Animal%in% c("C9","C10"),]
...
p7 = ggplot(df, aes(observed,predict)) +
  ...
  theme_bw()
p7
ggsave(paste0("output/Test_prediction_rare-l6", "-calf.png"), p7, height = 4, width = 4)
...
afex_plot(m1_otu, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Number of Predicted Genes", x = "Day") +
  theme(legend.position="none") 
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Day")
ggsave(paste0("output/calf_picrust_BC-ellipse_", my_column,"-pig.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
phenotypes <- read.table("calf_predictions.txt", header=TRUE, sep="\t")
str(phenotypes)

meta_phenotypes <- merge(metadata, phenotypes, by.x = "ID", by.y = "ID")
str(meta_phenotypes)

# These samples were not well predicted
meta_phenotypes <- meta_phenotypes[-which(meta_phenotypes$ID %in% c("C_d14_C5")), ]
meta_phenotypes <- meta_phenotypes[-which(meta_phenotypes$ID %in% c("C_d21_C10")), ]
...
ggsave(paste0("output/calf_bugbase_Anaerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/calf_bugbase_Aerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes, Aerobic_log = log10(Aerobic))

# Transformed Aerobic
T1_Aerobic <- mixed(Aerobic_log ~ Day + Animal + (Day||Animal), data = meta_phenotypes, method = "KR",
                    control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
afex_plot(T1_Aerobic, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of Log Aerobes", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_calf_bugbase_Aerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/calf_bugbase_F_Anaerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes,Facultatively_Anaerobic_sqrt = sqrt(Facultatively_Anaerobic))

# Transformed Facultatively Anaerobic
T1_F_Anaerobic <- mixed(Facultatively_Anaerobic_sqrt ~ Day + Animal + (Day||Animal), data = meta_phenotypes, method = "KR",
                        control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
afex_plot(T1_F_Anaerobic, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of Log Facultative Anaerobes", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_calf_bugbase_F_Anaerobic", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/calf_bugbase_Biofilms", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes, Forms_Biofilms_sqrt = sqrt(Forms_Biofilms))

# Transformed Forms_Biofilms
T1_Biofilms <- mixed(Forms_Biofilms_sqrt ~ Day + Animal + (Day||Animal), data = meta_phenotypes, method = "KR",
                     control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)
...
with(meta_phenotypes, shapiro.test(Forms_Biofilms_sqrt))

afex_plot(T1_Biofilms, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of Log Biofilm Formers", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_calf_bugbase_Biofilms", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/calf_bugbase_Gram_Negative", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
with(meta_phenotypes, shapiro.test(Gram_Positive_sqrt))
# Normal
# Sqrt transformation fixed it
...
ggsave(paste0("output/calf_bugbase_Gram_Positive", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

meta_phenotypes <- mutate(meta_phenotypes, Gram_Positive_sqrt = sqrt(Gram_Positive))

# Transformed Gram_Positive
T1_Gram_Positive <- mixed(Gram_Positive_sqrt ~ Day + Animal + (Day||Animal), data = meta_phenotypes, method = "KR",
                          control = lmerControl(optCtrl = list(maxfun=1e6)), expand_re=TRUE)

summary(T1_Gram_Positive)

anova(T1_Gram_Positive) 
plot(T1_Gram_Positive$full_model)
qqnorm(residuals(T1_Gram_Positive$full_model))
emm_options(lmer.df = "asymptotic") # also possible: 'satterthwaite', 'kenward-roger'
emm_t1_Gram_Positive<- emmeans(T1_Gram_Positive, "Day")
emm_t1_Gram_Positive
update(pairs(emm_t1_Gram_Positive), by = NULL, adjust = "holm")
with(meta_phenotypes, shapiro.test(Gram_Positive_sqrt))
# Sqrt transformation fixed it, p = 0.8334

afex_plot(T1_Gram_Positive, x = "Day", id = "ID", error = "between", dodge = 0.4, point_arg = list(size = 4), mapping="color") +
  theme_bw() + theme(legend.position="bottom") + 
  labs(y = "Relative Abundance of Log Gram Positive", x = "Day") +
  theme(legend.position="none") 

ggsave(paste0("output/T_calf_bugbase_Gram_Positive", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggsave(paste0("output/calf_bugbase_Contains_Mobile_Elements", ".pdf"), height=4, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches